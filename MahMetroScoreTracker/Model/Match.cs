...
namespace MahMetroScoreTracker.Model
{
    ...
    class Venue
    {
        ...
        public string VenueTime { get; set; }
    ...
    class Match
    {
        ...
        public Score FinalScore
        {
            get
            {
                return Scores.Where(s => !s.NotApplicable).FirstOrDefault(s => s.ScoreType == ScoreType.AfterExtraTime) ??
                    Scores.Where(s => !s.NotApplicable).FirstOrDefault(s => s.ScoreType == ScoreType.FullTime);
            }
        }
        ...
        public Score DecidingScore
        {
            get
            {
                return Scores.Where(s => !s.NotApplicable).FirstOrDefault(s => s.ScoreType == ScoreType.Penalties) ?? 
                    Scores.Where(s => !s.NotApplicable).FirstOrDefault(s => s.ScoreType == ScoreType.AfterExtraTime) ??
                    Scores.Where(s => !s.NotApplicable).FirstOrDefault(s => s.ScoreType == ScoreType.FullTime);
            }
        }
